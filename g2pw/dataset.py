...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_pos:
            # If use_pos is True but no pos_tags are provided (e.g., during inference),
            # we assign a default 'UNK' (unknown) pos_id.
            # This ensures the model receives the expected input tensor shape.
            pos_id = self.POS_TAGS.index(self.pos_tags[idx] if self.pos_tags is not None else 'UNK')
    ...
    def create_mini_batch(self, samples):
        ...
        if self.use_pos:
            # If use_pos is True, we must provide a pos_ids tensor,
            # even if it's just a default 'UNK' value.
