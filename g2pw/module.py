    ...
    def forward(self, input_ids, token_type_ids, attention_mask, phoneme_mask, char_ids, position_ids, pos_ids=None, label_ids=None, eps=1e-6):
        ...
        if self.use_pos:
            if pos_ids is not None:
                pred_pos_ids = pos_ids
            else:
                pred_pos_ids = torch.zeros_like(char_ids)
        else:
            pred_pos_ids = None
        ...
        if self.use_conditional:
            ...
            if self.use_pos and self.param_conditional['pos-linear']:
                affect_terms.append(self.pos_descriptor(pred_pos_ids))
            if self.use_pos and self.param_conditional['char+pos-second']:
                char_pos_ids = self._get_char_pos_ids(char_ids, pred_pos_ids)
                affect_terms.append(self.second_order_descriptor(char_pos_ids))
            if self.use_pos and self.param_conditional['char+pos-second_lowrank']:
                char_pos_ids = self._get_char_pos_ids(char_ids, pred_pos_ids)
                affect_terms.append(self.second_lowrank_descriptor(char_pos_ids))
            if self.use_pos and self.param_conditional['char+pos-second_fm']:
                affect_terms.append(
                    torch.sum(
            ...
            elif self.use_pos and self.param_conditional['fix_mode'] == 'count_distr:char+pos':
                char_pos_ids = self._get_char_pos_ids(char_ids, pred_pos_ids)
